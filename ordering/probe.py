...
args_parser.add_argument('--max_token_sent', default=50, help='maximum token for one sentence')
args_parser.add_argument('--batch_size', type=int, default=80, help='batch size')
...
# All models use average pooling computation
...
def read_data(fname):
    ...
    for datum in data:
        sentences.append(datum['sentences'])
...
def train(args, train_dataset, dev_dataset, test_dataset, model):
    ...
    for i in range(int(args.num_train_epochs)):
        ...
        dev_corr, pred = prediction(tokenizer, dev_dataset, model, args)
        if dev_corr > best_corr_dev:
            best_corr_dev = dev_corr
            dev_pred = pred
            test_acc, test_pred = prediction(tokenizer, test_dataset, model, args)
            best_corr_test = test_acc
            cur_patience = 0
            logger.info("Better, BEST Corr in DEV = %s & BEST Corr in test = %s.", best_corr_dev, best_corr_test)
        else:
            cur_patience += 1
            if cur_patience == args.patience:
                logger.info("Early Stopping Not Better, BEST Corr in DEV = %s & BEST Corr in test = %s.", best_corr_dev, best_corr_test)
                break
            else:
                logger.info("Not Better, BEST Corr in DEV = %s & BEST Corr in test = %s.", best_corr_dev, best_corr_test)

    return global_step, tr_loss / global_step, best_corr_dev, best_corr_test, dev_pred, test_pred
...
def to_print(arrs):
    tmp = []
    for arr in arrs:
        tmp.append('-'.join([str(a) for a in arr]))
    return '|'.join(tmp)
...
for idx in range(args.start,args.num_layers+1):
    ...
    print('Test set RankCorr', best_corr_test)
    f = open(output_file, 'a+')
    str_dev_pred  = to_print(dev_pred)
    str_test_pred = to_print(test_pred)
    f.write(f"{idx}, {best_corr_dev}, {best_corr_test}, {str_dev_pred}, {str_test_pred}\n")
    f.close()
