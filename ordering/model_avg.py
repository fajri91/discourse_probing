...
class ModelData():
    def __init__(self, args):
        ...
        self.MAX_TOKEN_SENT = args.max_token_sent

    def preprocess_one(self, sent, label):
        sent_subtoken_ids = [self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(t)[:self.MAX_TOKEN_SENT]) for t in sent]
        
        idx=len(sent_subtoken_ids)
        while idx < TOTAL_SENTENCE:
            label.append(TOTAL_SENTENCE)
            idx+=1
        return sent_subtoken_ids, label
    
    def preprocess(self, sents, labels):
        assert len(sents) == len(labels)
        output = []
        for idx in range(len(sents)):
            output.append(self.preprocess_one(sents[idx], labels[idx]))
        return output
...
class Model(nn.Module):
    def __init__(self, args, device):
        ...
        self.linear = nn.Linear(self.model.config.hidden_size, TOTAL_SENTENCE)
        self.dropout = nn.Dropout(0.2)
        self.loss = torch.nn.CrossEntropyLoss(ignore_index=TOTAL_SENTENCE, reduction='sum')
    ...
    def forward(self, src, mask_src, stack_index, stack_denominator):
        ...
        final_rep = self.dropout(stack_state)
        conclusion = self.linear(final_rep) #batch_size * 7 * 7
        return conclusion #batch_size * 7 * 7
    ...
    def get_loss(self, batch_data):
        ...
        return self.loss(output.view(-1,TOTAL_SENTENCE), label.view(-1))
    ...
    def predict_cor(self, src, mask_src, label, stack_index, stack_denominator):
        output = self.forward(src, mask_src, stack_index, stack_denominator)
        batch_size = output.shape[0]
        cls_mask = stack_denominator.view(batch_size, TOTAL_SENTENCE)
        cls_mask = 0 + (cls_mask != -1)
        cors = []
        preds = []
...
def prediction(tokenizer, dataset, model, args):
    ...
    for j in range(0, len(dataset), args.batch_size):
        ...
        cors, p = model.predict_cor(src, mask_src, label, stack_index, stack_denominator)
        rank_cors += cors
        preds += p
    return np.mean(rank_cors), preds
