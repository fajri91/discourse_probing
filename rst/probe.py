...
args_parser.add_argument('--max_token_word', default=250, help='maximum token for a single EDU')
...
args_parser.add_argument('--task', type=str, default='nuclearity', \
        choices=['nuclearity', 'relation'], help='select one of nuclearity of relation')
...
def read_data(fname, task):
    sentences = []
    labels = []
    df = pd.read_csv(fname)
    for idx, data in df.iterrows():
        sentences.append([data['edu1'], data['edu2']])
        if task == 'nuclearity':
            labels.append(data['nuclear'])
        elif task == 'relation':
...
trainset = read_data(args.train_data, args.task)
devset = read_data(args.dev_data, args.task)
testset = read_data(args.test_data, args.task)

label2id = create_vocab (trainset[1])
args.vocab_label_size = len(label2id)

trainset = trainset[0], convert_label2id (label2id, trainset[1])
devset = devset[0], convert_label2id (label2id, devset[1])
testset = testset[0], convert_label2id (label2id, testset[1])
...
output_file = args.output_folder+'/'+args.model_type+'_'+args.task+'.csv'
if args.start == 1:
